...
5. Prefer concrete facts from `YOLOBOX_CONTEXT_FILE` over generic assumptions. Mention the current project path, workdir, runtime, readonly/output behavior, network mode, Docker socket access, SSH agent access, and any relevant env keys or customization settings. In the script output, `Readonly project mode` is the yolobox launch mode and `Project writable now` is the current filesystem write check.
