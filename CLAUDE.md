...
- **Docker socket mount source on macOS**: On macOS, Docker runs inside a VM (Docker Desktop or Colima). Volume mount `-v` source paths are resolved **inside the VM**, not on the macOS host. The Docker socket inside any VM is always at `/var/run/docker.sock`. Host-side paths (like `~/.colima/default/docker.sock`) don't exist inside the VM. Solution: on macOS, always use `/var/run/docker.sock` as the mount source; check host-side socket paths only to verify Docker is running.
- **SSH agent socket on macOS**: Same problem as Docker socket — the host's `SSH_AUTH_SOCK` path (e.g., `/private/tmp/com.apple.launchd.XXX/Listeners`) doesn't exist inside the Docker VM. Solution: for Docker Desktop, use `/run/host-services/ssh-auth.sock` (automatically forwarded). For Colima, run `colima ssh -- printenv SSH_AUTH_SOCK` to get the VM-internal path, but this requires `forwardAgent: true` in `~/.colima/default/colima.yaml`.
